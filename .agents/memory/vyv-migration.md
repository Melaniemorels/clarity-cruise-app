...
- `.auth.*` → now bridges Clerk (`window.Clerk`), attaching the Clerk bearer token to API calls; the earlier custom email/password (scrypt/JWT) stub was replaced by real Clerk auth. Session shape still mimics Supabase (access_token, user) but tokens come from Clerk per request.
...
- **Out of scope (documented, not bugs):** health/device sync and watch delivery are intentionally NOT migrated (UI now gated as "Coming soon" — connect buttons disabled; real sync requires a native mobile app). Do not build these without an explicit request (billed import-free, "do not over-build"). Password recovery is handled by Clerk now.
