...
- **Simplified social layer (no Friends tab, no in-app messaging).** The Friends tab was removed from the bottom nav (nav is Home / Calendar / Capture / Profile with Explorer off); friend discovery lives inside the user search dialog: empty state offers an invite link (copy/share) and "Connect Contacts" via the browser Contact Picker API, backed by `POST /functions/v1/match-contacts` (matches emails against `app_users.email` and phones against `profiles.phone_number` with country-code-tolerant digit-suffix matching; returns only public profile fields, excludes caller, per-user rate limit 12/hour). "Plan Together" is mutual-confirm: the invitee's accept goes through `POST /functions/v1/plan-invite-respond`, which atomically flips the invite off "pending" (repeat/concurrent responses are no-ops), writes the event into BOTH calendars server-side (`external_id = social_plan:{id}`, creator-timezone wall clock; cross-user inserts are blocked in `db.ts`, hence server-side), confirms the plan, and notifies the creator. Instead of messaging, accepted friends' profiles show external contact buttons (`ContactFriendButtons.tsx`: sms:/tel:, FaceTime on Apple devices, WhatsApp elsewhere) when the friend has a phone number; users add their phone in the edit-profile dialog (`profiles.phone_number`). Tests: `artifacts/api-server/src/test/plan-invite.test.ts`.
